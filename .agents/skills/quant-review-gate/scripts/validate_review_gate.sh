...
require_grep 'blocked cheap checks' "$SKILL_FILE"
require_grep 'denied `rg`|unavailable.*bash|missing `pytest`' "$SKILL_FILE"
require_grep 'Win32 error 5' "$SKILL_FILE"
require_grep 'Select-String' "$SKILL_FILE"
require_grep 'routed_validator_requests|routed validator' "$SKILL_FILE"
require_grep 'quant-validator-approval' "$SKILL_FILE"
