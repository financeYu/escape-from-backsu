...
class CrossSectionalNormalizationConfig:
    ...
    ``min_count`` should normally come from
    ``Quant_mvp/config/thresholds.toml -> [quality].min_cross_section_count``.
    Winsorization and z-score clipping are optional and are counted in
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.winsorize_lower_pct is not None
            and self.winsorize_upper_pct is not None
            and self.winsorize_lower_pct > self.winsorize_upper_pct
        ):
            raise ValueError("winsorize_lower_pct cannot exceed winsorize_upper_pct.")
        if self.zscore_clip is not None and self.zscore_clip <= 0:
            raise ValueError("zscore_clip must be positive when provided.")
...
def load_cross_sectional_normalization_config(
    *,
    ...
    Robust z-score clipping is not configured yet, so it defaults to `None`
...
def robust_zscore_cross_sectional(
    ...
    *,
    ...
    finite_mask = _finite_mask(numeric)
    ...
    row_flags = _initial_value_flags(values, numeric, finite_mask)
    if valid_count < active_config.min_count:
        for index in values.index:
            row_flags[index].add("insufficient_cross_section")
        result["cross_sectional_quality_flag"] = _flags_to_series(row_flags, values.index)
        return result

    finite_values = numeric.loc[finite_mask].astype(float)
...
def _score_quality_flags(
    *,
    ...
    return _flags_to_series(flags, prepared.index)
...
def _flags_to_series(flags: dict[object, set[str]], index: pd.Index) -> pd.Series:
    return pd.Series([_format_flags(flags[item]) for item in index], index=index, dtype="string")
