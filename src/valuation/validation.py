...
STEP18_AVAILABILITY_DATE_COLUMNS = ("available_date", "as_of_date")
...
def _coerce_record(
    ...
    *,
    ...
    _assert_safe_symbol(symbol)
    company_name = _resolve_company_name(payload)
    metric_name = _required_text(payload, "metric_name").lower()
    spec = registry_by_name.get(metric_name)
    if spec is None:
        raise ValueError(f"Step 18 candidate metric is outside the allowed registry: {metric_name}")
    ...
    if metric_category != spec.metric_category:
        raise ValueError(
            ...
            f"{metric_name}: {metric_category} != {spec.metric_category}"
        )
    ...
    if metric_unit != spec.metric_unit:
        raise ValueError(
            ...
            f"{metric_name}: {metric_unit} != {spec.metric_unit}"
        )

    metric_value = _required_numeric(payload.get("metric_value"), field_name="metric_value")
    available_ts = _required_availability_date(payload)
    report_ts = _required_date(payload.get("report_date"), field_name="report_date")
    ...
    if evaluation_ts is not None:
        ...
        if available_ts > evaluation_ts:
            future_fields.append("available_date/as_of_date")
        if report_ts > evaluation_ts:
    ...
    return ValuationCandidateRecord(
        ...
        fiscal_period=_required_text(payload, "fiscal_period"),
        report_date=_date_string(report_ts),
        available_date=_date_string(available_ts),
        ...
        filing_date=_optional_date_string(_optional_date(payload.get("filing_date"), field_name="filing_date")),
        source_report_id=_optional_text(payload.get("source_report_id")),
        source_vendor=_optional_text(payload.get("source_vendor")),
        collected_at=_optional_date_string(_optional_date(payload.get("collected_at"), field_name="collected_at")),
    )
...
def _assert_required_candidate_fields(payload: Mapping[str, Any]) -> None:
    missing = [
        ...
        if field not in payload and field not in {"available_date", "company_name"}
    ]
    if "available_date" not in payload and "as_of_date" not in payload:
        missing.append("available_date/as_of_date")
...
def _assert_safe_symbol(symbol: str) -> None:
    if not re.fullmatch(r"\d{6}", symbol):
        raise ValueError("Step 18 candidate symbol must preserve a six-digit KOSPI code string.")
...
def _required_availability_date(payload: Mapping[str, Any]) -> pd.Timestamp:
    value = payload.get("available_date", payload.get("as_of_date"))
    if value is None or value is pd.NA or str(value).strip() == "":
        raise ValueError("Step 18 candidate available_date/as_of_date is required.")
    return _required_date(value, field_name="available_date/as_of_date")
