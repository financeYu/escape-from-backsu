...
이 문서는 현재 Step 15 support branch의 design-only 산출물이다.
...
이 문서는 기존 technical research/scoring 산출물과 미래 Step 17 conservative backtest 산출물 사이의 경계를 정의한다. 이 문서 자체는 `src/`, runtime pipeline, ranking output, report output, backtest engine, valuation/fundamental logic을 변경하지 않는다.
...
Local design-only confirmation for this branch:

- `WORKSPACE_MANIFEST.md` limits writes to this design doc and manifest.
- No source code under `src/` is changed by this task.
...
- trigger: design document created for Step 15 support branch
...
- dirty worktree isolation: design-only branch and manifest are separate from Step 15 implementation branches
