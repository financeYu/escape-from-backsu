...
def render_ingestion_report(
    ...
    return "\n".join(
        [
            *_ingestion_report_header_lines(
                run_id,
                run_timestamp,
                selected_sources,
                selected_query_set,
            ...
            *_ingestion_report_guardrail_lines(),
            *_ingestion_report_summary_lines(
                branch_counts,
                route_counts,
                collection_summary,
                enrichment_summary,
...
def _ingestion_report_header_lines(
    run_id: str,
    run_timestamp: Any,
    selected_sources: list[Any],
    selected_query_set: Any,
    raw_counts: dict[str, Any],
    relevance_rejected: Any,
    relevance_manual_review: Any,
    enrichment_target_count: Any,
    ...
    manual_count: int,
    retracted_count: int,
    unresolved_seeds: list[dict[str, Any]],
) -> list[str]:
    return [
...
def _ingestion_report_guardrail_lines() -> list[str]:
    return [
        "## 핵심 제한 사항",
...
def _ingestion_report_summary_lines(
