...
Candidate rows require `ticker`, `period`, `metric`, `value`, `filing_date`,
`availability_date`, `source_vendor`, `collected_at`, and either
`disclosure_id` or `source_report_id`. Legacy aliases are accepted only at
ingestion and are normalized before candidate reporting.
