...
require_grep 'BLOCKER_UNTIL_CONFIRMED_SOURCE' "$SCORE_CONTRACT"
...
require_grep 'BLOCKER_UNTIL_CONFIRMED_ADJUSTED_CLOSE_SOURCE' "$GATE1_DOC"
...
require_grep 'repo_availability_status = "BLOCKER_UNTIL_CONFIRMED_ADJUSTED_CLOSE_SOURCE"' "$GATE1_CONFIG"
...
if grep -Eiq "$trade_word_pattern" "$SCORE_CONTRACT" "$GATE1_DOC" "$GATE1_CONFIG" "$SKILL_FILE"; then
