# Valuation Review Skill Examples

이 문서는 메인 `AGENTS.md`에서 밸류 검토를 분리한 뒤, 언제 어떤 식으로
`../.agents/skills/valuation_review/SKILL.md`를 써야 하는지 보여주는 예시입니다.

## 핵심 원칙

메인 에이전트는 기술적 스코어 설계, 구현, 진단, 선택을 담당합니다.
밸류 판단은 `../.agents/skills/valuation_review/SKILL.md`가 담당합니다.
...
밸류 판단이 필요하면 point-in-time fundamental data를 확인한 뒤 valuation review skill로 넘겨야 합니다.
```

판정:
- 메인 에이전트: `reject as valuation`
- 가능한 대안: `technical drawdown / mean-reversion diagnostic`
- valuation review skill 호출: 아직 불필요

---

## 예시 2: valuation review skill로 넘겨야 하는 요청
...
```text
earnings yield는 valuation branch에 속하므로 valuation review skill에서 검토해야 합니다.
먼저 point-in-time earnings, market cap, filing date, stale-data policy가 있는지 확인해야 합니다.
```

valuation review skill이 확인할 항목:
- `ttm_net_income_pti`
- `market_cap_t`
...
그 다음 valuation review skill이 valuation_composite_score 가능성을 검토합니다.
마지막 adoption synthesis에서 두 결과를 명시적으로 결합합니다.
```

주의점:
- 기술 에이전트가 밸류 점수를 임의로 만들면 안 됩니다.
- valuation review skill이 기술 점수를 임의로 재해석하면 안 됩니다.
- 결합 규칙은 config에 있어야 하며 코드에 하드코딩하면 안 됩니다.

---

## 예시 5: valuation review skill을 호출하지 않아도 되는 요청
...
처리:
- 메인 기술 에이전트가 처리합니다.
- valuation review skill은 호출하지 않습니다.
...
기술 에이전트는 밸류를 만들지 않고, valuation review skill은 기술 신호를 재포장하지 않는다.
```
