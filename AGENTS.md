...
3a. Global implementation simplicity rule: write the simplest code that satisfies the approved contract and tests. Prefer existing patterns, plain data flow, small functions, and standard-library/project-local helpers; avoid new abstractions, frameworks, clever indirection, broad refactors, or configurable machinery unless they clearly remove real complexity or are required by the active contract.
...
Quant ownership topology: `chart_mvp` and `Quant_mvp/research_mvp` are
Quant_mvp subfunction lanes, not separate cross-project domains. `chart_mvp`
is the Quant chart/runtime/data-support lane, while `Quant_mvp/research_mvp`
is the Quant research-ingestion lane. They still require supervisor-approved
scope locks, generated-output boundaries, and owner-lane validation before
their outputs can feed v0.3 evidence, but Quant-local handoffs among these
lanes are not cross-project handoffs. `review_mvp` and root-owned policy/CI/Git
areas remain separate project/root boundaries.
...
For agent architecture redesign or replacement work, start with `.agents/skills/agent-coordinator/SKILL.md`, then `.agents/skills/agent-planner/SKILL.md`, then `.agents/skills/agent-plan-review/SKILL.md`, then `.agents/skills/agent-supervisor/SKILL.md`, then `.agents/skills/agent-worker-pool/SKILL.md`, then `.agents/skills/agent-reporter/SKILL.md`, then `.agents/skills/agent-replacement/SKILL.md`. The new architecture is the active root orchestration path; existing project-local gates remain domain authority as compatibility targets selected and bounded by the architecture chain. Do not delete or rename a legacy gate until its authority is represented in the new chain and validation passes.
For work that can be owned by a subproject or role worktree, route through `.agents/skills/subproject-delegation/SKILL.md` under the architecture chain: delegate subproject-contained exploration/implementation/validation to the owning lane, assign one writer per path, and return only compact `status`, `changed_scope`, `evidence`, and `next_request` summaries to root.
...
Validation environment: when Python validation, `pytest`, or pandas-dependent checks are required from root or any subproject, prefer the root workspace interpreter `.venv\Scripts\python.exe ...` and `.venv\Scripts\python.exe -m pytest ...`. Do not install pytest or pandas separately in subproject-local, system, or user-site paths. The root `.venv` owns the Codex sandbox ACL and routes Python temp files to the workspace-local `.pytest_tmp/` path. Use `.agents/skills/workspace-python-env/SKILL.md` whenever a task runs Python validation or fixes a missing-package environment blocker; treat system-Python `ModuleNotFoundError` as an environment blocker, not test evidence.
